...
export default function Index() {
  ...
  async function checkAddressAndSend(event) {
    ...
    if (addressValid) {
      ...
      setModalOpen(true);
      setProcessStage(0);
      const score = await createToken();
      console.log({ score });
      if (parseFloat(score) && score < 0.5) {
        setModalOpen(false);
        window.alert("YOU ARE NOT HUMAN - according to us");
        return false;
      }
      setProcessStage(1);
      fetcher.submit(
  ...
  const createToken = () => {
    return new Promise((res, rej) => {
      window.grecaptcha.enterprise.ready(async () => {
        const token = await window.grecaptcha.enterprise.execute(CAPTCHA_ID, {
          action: "REQUEST_TRIVIA_TOKEN",
        });
        console.log({ token });
        const postResponse = await fetch("/api/recaptcha", {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            token,
            actionName: "REQUEST_TRIVIA_TOKEN",
          }),
        });
        const score = await postResponse.json();
        res(score);
      });
    });
  };
        ...
        <div className="modal-box">
          {processStage === 0 && (
            <div className="pending flex items-center">
              ...
              <span className="pl-4">Checking to see if you're human</span>
            </div>
          )}
          {processStage === 1 && (
            <div className="pending flex items-center">
